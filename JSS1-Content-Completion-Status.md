...
Garment Making, Beauty and Cosmetology, Livestock Farming, and Solar Photovoltaic Installation
and Maintenance are tagged `legacy`.
...
the summarized fetch, before writing the data file). JSS2 and JSS3 still carry the prior
13-subject catalogue (this session covered JSS1 only), so this gap no longer applies uniformly
across JSS1-3 — JSS1 is now ahead by seven subjects. **2 subjects remain missing**: the other 2
named trade options (Computer Hardware and GSM Repairs, Horticulture and Crop Production).
Neither has been sourced or attempted yet, but given that Solar Photovoltaic Installation and
Maintenance, Fashion Design and Garment Making, Beauty and Cosmetology, and Livestock Farming
were all found on the same `schemeofwork.com` "JSS1 Scheme of Work Unified" page's Trade
Subjects section, the remaining 2 are likely to be found there too. This is still a larger gap
than any single Primary 4-6 grade (which topped out at 1 missing subject each) but has been
narrowing quickly this session.
